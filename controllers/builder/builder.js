...
exports.moveToTrash = async (req, res) => {
    try {
        ...
        let formId = mongoose.Types.ObjectId(req.params.id)
        let form = await Form.findOne({ _id: formId })
...
exports.createDigitalForm = async (req, res) => {
    ...
    try {
        if (formType == "document") {
            ...
            data.map(async (ele) => {
                let documentForm = new uploadDocument
                documentForm.document_name = ele.documentData.document_name
                documentForm.subFolderId = ele.documentData.subFolderId
                documentForm.userId = ele.documentData.userId
                documentForm.document = ele.documentData.document
                documentForm.__v = ele.documentData.__v
