...
const Form=require("../../models/builder/Form")
...
exports.createRolesList = async (req, res) => {
    ...
    let digitalId = mongoose.Types.ObjectId(req.body.digitalId)
    let documentId = mongoose.Types.ObjectId(req.body.documentId)
    let taskId = mongoose.Types.ObjectId(req.body.taskId)
    let digital = await Form.exists({ _id: { $in: digitalId } });
    let document = await uploadDocument.exists({ _id: { $in: documentId } });
    let task = await Task.exists({ _id: { $in: taskId } });
    try {
        if (digital) {
            ...
            RolesListObj.save((err, data) => {
                if (err) {
                    ...
                    res.send({ 'msg': 'Roles-List info document add successfully.',data, 'success': true })
                }
            })
        } else if (task) {
            let RolesListObj = new RolesList(req.body)
...
exports.submitForm = async (req, res) => {
    ...
    let taskId = mongoose.Types.ObjectId(req.body.taskId);
    let digitalId = mongoose.Types.ObjectId(req.body.digitalId);
    let formType = mongoose.Types.ObjectId(req.params.formType);
    try {
        if (formType == "document") {
            ...
            documentData.map(async (ele) => {
                if (documentId == (ele.documentData._id)) {
                    let result = await uploadDocument.updateOne({ _id: documentId }, { $set: { isSubmit: true } })
            ...
            digitalData.map(async (ele) => {
                if (digitalId == (ele.digitalData._id)) {
                    let result = await Form.updateOne({ _id: digitalId }, { $set: { isSubmit: true } })
                    if (result.nModified === 1) {
                        return res.send("form submit")
                    } else {
                        return res.send("form hasn't been submit")
        ...
        if (formType == "document") {
            ...
            documentData.map(async (ele) => {
                if (ele.documentData.isSubmit) {
                    let result = await uploadDocument.updateOne({ _id: ele.documentData._id }, { $set: { isApprove: true } })
                    if (result.nModified === 1) {
            ...
            digitalData.map(async (ele) => {
                if (ele.digitalData.isSubmit) {
                    let result = await Form.updateOne({ _id: ele.digitalData._id }, { $set: { isApprove: true } })
                    if (result.nModified === 1) {
                        return res.send("form approve")
                    } else {
                        return res.send("form hasn't been approve")
